...
exports.userRegister = async (email, password, gender) => {
    try {
...
exports.createToken = (user) => {
    ...
    // jwtPromisify({ id: user._id, email: user.email }, constants.SECRET, { expiresIn: '5d' });
    let token = new Promise((resolve, reject) => {
        jwt.sign({ id: user._id, email: user.email }, constants.SECRET, { expiresIn: '5d' }, (err, decodedToken) => {
            if (err) {
