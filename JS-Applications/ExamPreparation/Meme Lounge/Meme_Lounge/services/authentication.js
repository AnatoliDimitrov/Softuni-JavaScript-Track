...
async function getMostRecent() {
    try {
        let result = await jsonRequest(constants.MOST_RECENT);
        return result;
    } catch (err) {
        alert(err);
    }
...
async function getMy(userId) {
    try {
        let result = await jsonRequest(`${constants.BASE}${`?where=_ownerId%3D%22${userId}%22&sortBy=_createdOn%20desc`}`);
        return result;
    } catch (err) {
        alert(err);
    }
...
function fillLocaleStorage(loginResult) {
    ...
    localStorage.setItem('gender', loginResult.gender);
}
